...
def plot_metrics(metrics_dict, model_name, loc=None):
    ...
    plt.xticks(X_axis, X_names, rotation=45)
    ...
    plt.tight_layout()
    savefig(f'graphic_{model_name}_mendeley_mult')
    return scores_dict
...
def plot_confusion_matrix(cm, model_name, dataname='', normalize=False, cmap='Blues'):
    ...
    plt.grid(False)
    plt.tight_layout()
    savefig(f'confusion_matrix_{model_name}_{dataname}_mendeley_mult')
...
scores_rf = plot_metrics(metrics_rf, 'rf')
